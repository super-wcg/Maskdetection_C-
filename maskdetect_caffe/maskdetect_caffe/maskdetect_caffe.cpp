...
void maskdetection::init_mask_modle() {
	maskdetection::PNet_ = cv::dnn::readNetFromCaffe("face_mask_detection.prototxt", "face_mask_detection.caffemodel");
}

vector<vector<float>> maskdetection::generate_anchors(const vector<float>& ratios, const vector<int>& scales, vector<float>& anchor_base)
{
...
vector<cv::Rect2f> maskdetection::decode_bbox(vector<vector<float>>& anchors, float* raw)
{
...
bool maskdetection::mask_detection(cv::Mat img) {
	vector<float> ratios;
...
int main(int argc, const char* argv[]) {
	...
	cv::Mat rimg;
	rimg = img(rect);
	//resize(img, rimg, cv::Size(260, 260));
	imwrite("1.jpg", rimg);
	bool ans;
	ans = m.mask_detection(rimg);
